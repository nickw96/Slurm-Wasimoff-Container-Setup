...
def analyse_cluster(report_name : str, observation_start : datetime, observation_end : datetime, succesful_tasks_total : int,
                    node_wasimoff_usage : list, node_slurm_usage : list, node_prolog_usage : list, node_epilog_usage : list, 
                    node_idle_usage : list, num_slurm_jobs : int, failed_tasks_total : int):
...
Start of observation:                               {observation_start.strftime(PRINT_TIME_FORMAT)}
End of observation:                                 {observation_end.strftime(PRINT_TIME_FORMAT)}
...
def print_activity_chart(report_name : str, observation_start : datetime, observation_end : datetime, node_time_lines : list):
    ...
    ax.legend(handles=patches, labels=['slurm', 'wasi_complete', 'wasi_abort', 'idle', 'prolog', 'epilog'], fontsize=12, bbox_to_anchor=(1.13, 0.81))
        ...
        for entry in entries:
            ...
            with open(entry.path, 'r', encoding='utf-8') as file:
                ...
                for i in range(0,num_nodes_in_job):
                    split_line = lines[i].split()
                    nodes_slurm_jobs[mapping[split_line[0]]].append({'start' : datetime.strptime(f'{split_line[1]} {split_line[2]}', TIMESTAMP_READ_FORMAT), 'state' : 'slurm'})
                if slurmstepd_lines:
                    for slurmstepd_line_index in slurmstepd_lines:
                        # get cancellation time
                        for i in range(0, num_nodes_in_job):
                            split_line = lines[slurmstepd_line_index + i].split()
                            nodes_slurm_jobs[mapping[split_line[0]]][-1]['end'] = datetime.strptime(f'{split_line[1]} {split_line[2]}', TIMESTAMP_READ_FORMAT)
                ...
                if name in entry.name:
                    with open(entry.path, 'r', encoding='utf-8') as file:
                        ...
                        for i in range(0,prologs_in_file):
                            ...
                            nodes_prologs[name].append({'start' : datetime.strptime(f'{split_line[1]} {split_line[2]}', TIMESTAMP_READ_FORMAT), 'state' : 'prolog'})
                            tmp = lines[i * 2 + 1].strip()
                            split_line = tmp.split()
                            nodes_prologs[name][-1]['end'] = datetime.strptime(f'{split_line[1]} {split_line[2]}', TIMESTAMP_READ_FORMAT)
                ...
                if name in entry.name:
                    with open(entry.path, 'r', encoding='utf-8') as file:
                        ...
                        for i in range(0,prologs_in_file):
                            tmp = lines[i * 2].strip()
                            split_line = tmp.split()
                            nodes_epilogs[name].append({'start' : datetime.strptime(f'{split_line[1]} {split_line[2]}', TIMESTAMP_READ_FORMAT), 'state' : 'epilog'})
                            tmp = lines[i * 2].strip()
                            split_line = tmp.split()
                            nodes_epilogs[name][-1]['end'] = datetime.strptime(f'{split_line[1]} {split_line[2]}', TIMESTAMP_READ_FORMAT)
    ...
    with open(log, 'r', encoding='utf-8') as logfile:
        ...
        for line in lines:
            ...
            if "Task" in line and "completed" in line:
                succesful_tasks += 1
                tasks_per_period[-1][line_split[-2]]['end'] = datetime.strptime(f"2025 {line_split[0]} {line_split[1]} {line_split[2]}",
                        JOURNAL_TIME_READ_FORMAT)
                ...
                for task_id in tasks_per_period[-1].keys():
                    if tasks_per_period[-1][task_id]['state'] == 'wasi_abort':
                        tasks_per_period[-1][task_id]['end'] = datetime.strptime(f"2025 {line_split[0]} {line_split[1]} {line_split[2]}",
                        JOURNAL_TIME_READ_FORMAT)
    ...
    with open(f"{log}.csv", 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for time_slot in time_line:
            csvwriter.writerow({
                ...
                'start' : time_slot['start'].strftime(PRINT_TIME_FORMAT),
                'end' : time_slot['end'].strftime(PRINT_TIME_FORMAT),
